...
export default {
    async created() {
        ...
        this.form = await this.getStudentsByIdAsync(this._id);
        this.viceForm = this.form;
        this.getSubjectsAsync();
        if (this.form.subjectsId) {
            this.getClassBySubjectIdAsync(this.form.subjectsId);
        }
    },
    data() {
        return {
    ...
    methods: {
        ...mapActions(["getStudentsByIdAsync",'editStudentAsync']),
        ...
        async onSubmit() {
            const data = await this.editStudentAsync({...this.form,imagePath:this.imageUrl});
            if(data){
                this.$message({
                    message: "修改成功",
                    type: "success",
